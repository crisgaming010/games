...
// ============================================================
export async function firebaseLogin(email: string, password: string): Promise<[string | null, string, string]> {
  const cleanEmail = email.trim();
  const cleanPass = password.trim();
  // Try CPM 1 key first (most accounts), then fallback to CPM 2 key
  const keys = [_CPM1_FB_KEY, FK];
  ...
  for (const key of keys) {
    try {
      const resp = await fetch(`https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=${key}`, {
        ...
        body: JSON.stringify({ email: cleanEmail, password: cleanPass, returnSecureToken: true, clientType: "CLIENT_TYPE_ANDROID" }),
      });
      const data: any = await resp.json();
      if (resp.status === 200 && data.idToken) {
        return [data.idToken, data.localId || "", key];
      }
      lastErr = data?.error?.message || lastErr;
      if (lastErr === "INVALID_PASSWORD") {
        return [null, "INVALID_PASSWORD", key];
      }
    } catch (e: any) {
      lastErr = e.message || String(e);
    }
  }
  return [null, lastErr, _CPM1_FB_KEY];
}

export async function apiLoadRecord(email: string, password: string): Promise<AnyDict> {
  const [idToken, firebaseUid, usedKey] = await firebaseLogin(email, password);
  if (!idToken) return { success: false, message: firebaseUid };
  try {
    const resp = await fetch(LOAD_URL, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ email: email.trim(), password: password.trim(), uid: firebaseUid, fk: usedKey }),
    });
...
<body class="min-h-screen p-4 md:p-8">
  <div class="max-w-5xl mx-auto space-y-6">
    ...
    <!-- TAB 4: UNLOCKS & EXTRAS (ALL FEATURES) -->
    <section id="tab-unlocks" class="hidden space-y-6">
      <div class="card p-6 rounded-2xl space-y-6">
        <div>
          <h2 class="text-lg font-bold text-white mb-1">⚡ Performance & Customizations</h2>
          <p class="text-xs text-slate-400">I-click ang feature at pindutin ang Save Account para ma-apply sa laro.</p>
        </div>

        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
          <button onclick="unlockExtra('w16')" class="p-3 bg-slate-800/80 hover:bg-slate-700 rounded-xl text-center border border-indigo-500/30 font-medium text-sm text-indigo-400 flex flex-col items-center justify-center space-y-1">
            <span class="text-xl">🏎️</span>
            <span>W16 Engine</span>
          </button>
          <button onclick="unlockExtra('smoke')" class="p-3 bg-slate-800/80 hover:bg-slate-700 rounded-xl text-center border border-slate-600/40 font-medium text-sm text-slate-200 flex flex-col items-center justify-center space-y-1">
            <span class="text-xl">💨</span>
            <span>Tire Smoke</span>
          </button>
          <button onclick="unlockExtra('headlights')" class="p-3 bg-slate-800/80 hover:bg-slate-700 rounded-xl text-center border border-amber-500/30 font-medium text-sm text-amber-300 flex flex-col items-center justify-center space-y-1">
            <span class="text-xl">💡</span>
            <span>Custom Headlights</span>
          </button>
          <button onclick="unlockExtra('wheels')" class="p-3 bg-slate-800/80 hover:bg-slate-700 rounded-xl text-center border border-emerald-500/30 font-medium text-sm text-emerald-400 flex flex-col items-center justify-center space-y-1">
            <span class="text-xl">🛞</span>
            <span>Unlock 100 Wheels</span>
          </button>
          <button onclick="unlockExtra('fuel')" class="p-3 bg-slate-800/80 hover:bg-slate-700 rounded-xl text-center border border-teal-500/30 font-medium text-sm text-teal-300 flex flex-col items-center justify-center space-y-1">
            <span class="text-xl">⛽</span>
            <span>Unlimited Fuel</span>
          </button>
          <button onclick="unlockExtra('nodamage')" class="p-3 bg-slate-800/80 hover:bg-slate-700 rounded-xl text-center border border-blue-500/30 font-medium text-sm text-blue-300 flex flex-col items-center justify-center space-y-1">
            <span class="text-xl">🛡️</span>
            <span>No Damage / Shield</span>
          </button>
          <button onclick="unlockExtra('horns')" class="p-3 bg-slate-800/80 hover:bg-slate-700 rounded-xl text-center border border-yellow-500/30 font-medium text-sm text-yellow-300 flex flex-col items-center justify-center space-y-1">
            <span class="text-xl">📢</span>
            <span>Custom Horns</span>
          </button>
          <button onclick="unlockExtra('animations')" class="p-3 bg-slate-800/80 hover:bg-slate-700 rounded-xl text-center border border-purple-500/30 font-medium text-sm text-purple-300 flex flex-col items-center justify-center space-y-1">
            <span class="text-xl">🕺</span>
            <span>All Animations</span>
          </button>
          <button onclick="unlockExtra('perks')" class="p-3 bg-slate-800/80 hover:bg-slate-700 rounded-xl text-center border border-cyan-500/30 font-medium text-sm text-cyan-300 flex flex-col items-center justify-center space-y-1">
            <span class="text-xl">⭐</span>
            <span>All Perks</span>
          </button>
          <button onclick="unlockExtra('police_all')" class="p-3 bg-slate-800/80 hover:bg-slate-700 rounded-xl text-center border border-sky-500/30 font-medium text-sm text-sky-400 flex flex-col items-center justify-center space-y-1">
            <span class="text-xl">🚓</span>
            <span>Police All Cars</span>
          </button>
          <button onclick="unlockExtra('sirens')" class="p-3 bg-slate-800/80 hover:bg-slate-700 rounded-xl text-center border border-cyan-500/30 font-medium text-sm text-cyan-400 flex flex-col items-center justify-center space-y-1">
            <span class="text-xl">🚨</span>
            <span>Police Sirens/Lights</span>
          </button>
          <button onclick="unlockExtra('chrome_all')" class="p-3 bg-slate-800/80 hover:bg-slate-700 rounded-xl text-center border border-gray-400/40 font-medium text-sm text-white flex flex-col items-center justify-center space-y-1">
            <span class="text-xl">✨</span>
            <span>Chrome Specular All</span>
          </button>
          <button onclick="unlockExtra('bumpers_all')" class="p-3 bg-slate-800/80 hover:bg-slate-700 rounded-xl text-center border border-orange-500/30 font-medium text-sm text-orange-400 flex flex-col items-center justify-center space-y-1">
            <span class="text-xl">🛠️</span>
            <span>All Bumpers</span>
          </button>
          <button onclick="unlockExtra('paidhouse')" class="p-3 bg-slate-800/80 hover:bg-slate-700 rounded-xl text-center border border-green-500/30 font-medium text-sm text-green-400 flex flex-col items-center justify-center space-y-1">
            <span class="text-xl">🏠</span>
            <span>Paid House</span>
          </button>
          <button onclick="unlockExtra('allhouses')" class="p-3 bg-slate-800/80 hover:bg-slate-700 rounded-xl text-center border border-emerald-500/30 font-medium text-sm text-emerald-300 flex flex-col items-center justify-center space-y-1">
            <span class="text-xl">🏡</span>
            <span>Unlock All Houses</span>
          </button>
          <button onclick="unlockExtra('alllevels')" class="p-3 bg-slate-800/80 hover:bg-slate-700 rounded-xl text-center border border-amber-500/30 font-medium text-sm text-amber-400 flex flex-col items-center justify-center space-y-1">
            <span class="text-xl">🏆</span>
            <span>Complete 50 Levels</span>
          </button>
          <button onclick="unlockExtra('allclothes')" class="p-3 bg-slate-800/80 hover:bg-slate-700 rounded-xl text-center border border-pink-500/30 font-medium text-sm text-pink-400 flex flex-col items-center justify-center space-y-1">
            <span class="text-xl">👔</span>
            <span>Unlock All Clothes</span>
          </button>
        </div>
      </div>
    </section>
  ...
  <script>
    ...
    function unlockExtra(type) {
      if (!currentAccount || !currentAccount.record) return showToast('Mag-login muna bago mag-apply ng features!', true);
      const rec = currentAccount.record;
      rec.flags = rec.flags || {};

      switch(type) {
        case 'w16':
          rec.flags.w16 = '1';
          if (Array.isArray(rec.cars)) {
            rec.cars.forEach(c => { if (c) c.engineID = 5; });
          }
          showToast('🏎️ W16 Engine injected! Pindutin ang Save Account.');
          break;
        case 'smoke':
          rec.flags.smoke = '1';
          showToast('💨 Tire Smoke unlocked! Pindutin ang Save Account.');
          break;
        case 'headlights':
          rec.flags.headlights = '1';
          showToast('💡 Headlights feature unlocked! Pindutin ang Save Account.');
          break;
        case 'wheels':
          rec.wheels = Array.from({length: 100}, (_, i) => i + 1);
          showToast('🛞 100 Wheels unlocked! Pindutin ang Save Account.');
          break;
        case 'fuel':
          rec.flags.fuel = '1';
          showToast('⛽ Unlimited Fuel enabled! Pindutin ang Save Account.');
          break;
        case 'nodamage':
          rec.flags.nodamage = '1';
          showToast('🛡️ No Damage mode enabled! Pindutin ang Save Account.');
          break;
        case 'horns':
          rec.flags.horns = '1';
          showToast('📢 All Horns unlocked! Pindutin ang Save Account.');
          break;
        case 'animations':
          rec.flags.animations = '1';
          showToast('🕺 All Animations unlocked! Pindutin ang Save Account.');
          break;
        case 'perks':
          rec.flags.perks = '1';
          showToast('⭐ All Perks unlocked! Pindutin ang Save Account.');
          break;
        case 'police_all':
          if (Array.isArray(rec.cars)) {
            rec.cars.forEach(c => { if (c) c.police = true; });
          }
          rec.boughtPoliceLights = Array.from({length: 50}, (_, i) => i + 1);
          rec.boughtPoliceSirens = Array.from({length: 50}, (_, i) => i + 1);
          showToast('🚓 Police mode added to all cars! Pindutin ang Save Account.');
          break;
        case 'chrome_all':
          if (Array.isArray(rec.cars)) {
            rec.cars.forEach(c => {
              if (c) {
                c.specular = [1.0, 1.0, 1.0, 1.0];
                c.reflection = 1.0;
              }
            });
          }
          showToast('✨ Chrome finish applied to all cars! Pindutin ang Save Account.');
          break;
        case 'bumpers_all':
          if (Array.isArray(rec.cars)) {
            rec.cars.forEach(c => {
              if (c) c.bumpers = Array.from({length: 10}, (_, i) => i + 1);
            });
          }
          showToast('🛠️ All Bumpers unlocked! Pindutin ang Save Account.');
          break;
        case 'paidhouse':
          rec.flags.paidhouse = '1';
          showToast('🏠 Paid House unlocked! Pindutin ang Save Account.');
          break;
        case 'allhouses':
          rec.flags.allhouses = '1';
          showToast('🏡 All Houses unlocked! Pindutin ang Save Account.');
          break;
        case 'sirens':
          rec.boughtPoliceLights = Array.from({length: 50}, (_, i) => i + 1);
          rec.boughtPoliceSirens = Array.from({length: 50}, (_, i) => i + 1);
          showToast('🚨 Police Sirens & Lights unlocked! Pindutin ang Save Account.');
          break;
        case 'alllevels':
          rec.LevelsDoneTime = Array(50).fill(1.0);
          showToast('🏆 All 50 Levels completed! Pindutin ang Save Account.');
          break;
        case 'allclothes':
          const male = [], female = [];
          for (let i = 1; i < 200; i++) {
          ...
          showToast('👔 All Clothes unlocked! Pindutin ang Save Account.');
          break;
        default:
          showToast('Unknown feature');
      }
    }
    }
  </script>
